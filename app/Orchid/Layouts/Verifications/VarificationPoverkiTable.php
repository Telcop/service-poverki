...
class VarificationPoverkiTable extends Table
{
    /**
     * Data source.
     *
                    ...
                    DropDown::make()
                        ...
                        ->list([
                            ...
                            ModalToggle::make(__('Edit the verification'))
                                ...
                                ->method('editVerification')
                                ->modalTitle('Редактирование поверки ID = ' . $working->id)
                                ->asyncParameters(['id' => $working->id])
                                ->icon('bs.pencil'),
                            Button::make(__('Return the verification'))
                                ->method('returnVerification', ['id' => $working->id])
                                ->icon('bs.arrow-90deg-left')
                                ->confirm('Вы точно хотите вернуть поверку ID = ' . $working->id . ' в предыдущий этап?')
                        ]))
                // ->cantHide(),
        ];
    }
